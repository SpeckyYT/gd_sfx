...
{
    ComboBox::from_label(T::localized_enum())
...
fn select_gd_folder(ui: &mut Ui, app_state: &mut AppState) {
    ...
    ui.horizontal_wrapped(|ui| {
        let select_button = Button::new(t!("settings.gd_folder.select"));
        let response = ui.add_enabled(!app_state.is_tool_running(), select_button)
        ...
        if ui.button(t!("settings.gd_folder.open")).clicked() {
            if cfg!(target_os = "windows") {
                let _ = Command::new("explorer")
                    .arg(&app_state.settings.gd_folder)
                    .spawn();
            }
