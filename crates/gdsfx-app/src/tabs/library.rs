...
use crate::backend::{AppState, LibraryPage, search::{ListedMode, MusicFilters}, settings::SearchFilterMode};
...
static MUSIC_VIRTUAL_LIST: LazyLock<Mutex<VirtualList>> = LazyLock::new(Mutex::default);
...
fn render_music_library(ui: &mut Ui, app_state: &mut AppState, library: &MusicLibrary) {
    ...
    ScrollArea::vertical().auto_shrink(false).show(ui, |ui| {
        match app_state.music_filters.listed_mode {
            ListedMode::Listed => {
                let mut songs: Vec<_> = library.songs.values().filter(|song| {
                ...
                MUSIC_VIRTUAL_LIST.lock().ui_custom_layout(ui, songs.len(), |ui, start_index| {
                    layout::add_music_button(ui, app_state, songs[start_index]);
                    1
                });
            }
            ListedMode::Unlisted => {
