...
const SOURCE_FILE: &str = files::workspace_path!("locales/en_US.json");
const TARGET_FILE: &str = files::workspace_path!(".vscode/schemas/locale_schema.json");
...
pub fn build() {
    let source_file: Map<String, _> = files::read_json(SOURCE_FILE).unwrap();
    ...
    files::create_parent_dirs(target_file).unwrap();
    files::write_file(target_file, formatted_template).unwrap();
}
