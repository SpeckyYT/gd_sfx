...
lazy_static!{
    pub static ref GD_FOLDER: PathBuf = {
        if cfg!(target_os = "windows") {
            PathBuf::from(env::var("localappdata").expect("No local app data directory"))
                .join("GeometryDash")
        }
            ...
            let home_path = env::var("HOME").expect("home directory ENV variable not set");
            let home_path = Path::new(&home_path);

            let possible_gd_paths = [
            ...
            possible_gd_paths.iter()
                .map(|path| home_path.join(path))
                .find(|path| path.exists())
                .expect("No GD path found")
        }
