...
lazy_static!{
    pub static ref EXISTING_SOUND_FILES: Arc<Mutex<HashSet<u32>>> = Default::default();
}

pub fn add_file_to_stats(id: u32) {
    EXISTING_SOUND_FILES.lock().insert(id);
}

pub fn remove_file_from_stats(id: u32) {
...
pub fn add_existing_sfx_files() {
    if let Ok(readdir) = GD_FOLDER.read_dir() {
        readdir.flatten()
            ...
            .map(|s| s[1..s.len()-4].parse().unwrap())
